...
def GenerateAnchor(anc, grid):
  ...
  A = anc.shape[0]
  B, H, W = grid.shape[:3]
  anc_ = anc.reshape(1, A, 1, 1, 2)
  grid_ = grid.reshape(B, 1, H, W, 2)
  anchors = torch.cat(
    (grid_[..., 0:2] - anc_[..., 0:2] / 2.0,
     grid_[..., 0:2] + anc_[..., 0:2] / 2.0),
    dim=-1
    )
  ##############################################################################
...
def GenerateProposal(anchors, offsets, method='YOLO'):
  ...
  xca = (anchors[..., 2] + anchors[..., 0]) / 2.0
  yca = (anchors[..., 3] + anchors[..., 1]) / 2.0
  ...
  if method == 'YOLO':
    ...
    xcp = xca + offsets[..., 0] * wa
    ycp = yca + offsets[..., 1] * ha
  proposals[..., 0] = xcp - wp / 2.0
  proposals[..., 1] = ycp - hp / 2.0
  proposals[..., 2] = xcp + wp / 2.0
  proposals[..., 3] = ycp + hp / 2.0
  # for i in range(4):
  #   wha = wa if i in (0, 2) else ha
  #   tmp = 1.0 if i in (0, 1) else -1.0
  #   ii = i % 2
  #   proposals[..., i] = (anchors[..., i] 
  #       + wha / 2.0 * (1.0 - torch.exp(offsets[..., ii+2])) * tmp)
  #   if method == 'YOLO':
  #       proposals[..., i] += offsets[..., ii]
  #   else:
  #       proposals[..., i] += offsets[..., ii] * wha
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def IoU(proposals, bboxes):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  B, A, H, W = proposals.shape[:4]
  N = bboxes.shape[1]
  proposals_ = proposals.reshape(B, A*H*W, 1, 4)
  bboxes_ = bboxes[..., :4].reshape(B, 1, N, 4)
  area_p = ((proposals_[..., 2] - proposals_[..., 0]) 
    * (proposals_[..., 3] - proposals_[...,1]))
  area_b = ((bboxes_[..., 2] - bboxes_[..., 0]) 
    * (bboxes_[..., 3] - bboxes_[...,1]))
  dx = (torch.min(proposals_[..., 2], bboxes_[..., 2]) 
    - torch.max(proposals_[..., 0], bboxes_[..., 0]))
  dx = torch.clamp(dx, 0)
  dy = (torch.min(proposals_[..., 3], bboxes_[..., 3]) 
    - torch.max(proposals_[..., 1], bboxes_[..., 1]))
  dy = torch.clamp(dy, 0)
  area_i = dx * dy
  iou_mat = area_i / (area_p + area_b - area_i)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return iou_mat
...
class PredictionNetwork(nn.Module):
  ...
  def forward(self, features, pos_anchor_idx=None, neg_anchor_idx=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    B, _, H, W = features.shape[:4]
    A, C = self.num_anchors, self.num_classes
    outputs = self.pred_layer(features) # B, 5*A+C, H, W
    anchor_data = outputs[:, :5*A, ...].reshape(B, A, 5, H, W)
    anchor_data[:, :, 1:3, ...] = torch.sigmoid(anchor_data[:, :, 1:3, ...]) - 0.5
    anchor_data[:, :, 0, ...] = torch.sigmoid(anchor_data[:, :, 0, ...])
    all_scores = outputs[:, -C:, ...]
    if pos_anchor_idx is not None and neg_anchor_idx is not None:
        pos_anchors = self._extract_anchor_data(anchor_data, pos_anchor_idx)
        neg_anchors = self._extract_anchor_data(anchor_data, neg_anchor_idx)
        class_scores = self._extract_class_scores(all_scores, pos_anchor_idx)
        offsets = pos_anchors[:, 1:]
        conf_scores = torch.cat(
            (pos_anchors[:, 0:1], neg_anchors[:, 0:1]), dim=0)
    else:
...
class SingleStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    grid_list = GenerateGrid(B).to(bboxes)
    anchors = GenerateAnchor(self.anchor_list.to(bboxes), grid_list)
    ...
    GT_class, 
    _, 
    _) = ReferenceOnActivatedAnchors(
    anchors, bboxes, grid_list, iou_mat, neg_thresh=0.2, method='YOLO'
    )
    (conf_scores, 
    offsets, 
    class_scores) = self.pred_network(features, pos_anc_idx, neg_anc_idx)
    conf_loss = ConfScoreRegression(conf_scores, GT_conf_scores)
    reg_loss = BboxRegression(offsets, GT_offsets)
    anc_per_img = torch.prod(torch.tensor(anchors.shape[1:-1]))
    cls_loss = ObjectClassification(
        class_scores, GT_class, B, anc_per_img, pos_anc_idx
        )
    total_loss = w_conf * conf_loss + w_reg * reg_loss + w_cls * cls_loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
        ...
        (conf_scores, # B, A, H, W
        offsets, # B, A, 4, H, W
        class_scores) = self.pred_network(features) # B, C, H, W
        proposals = GenerateProposal(anchors, offsets.permute(0, 1, 3, 4, 2)) # B, A, H, W, 4
        for b in range(B):
            conf_score = conf_scores[b].reshape(-1) # A * H * W
            proposal = proposals[b].reshape(-1, 4) # A * H * W, 4
            class_score = class_scores[b] # C, H, W
            class_ = class_score.argmax(dim=0, keepdim=True) # 1, H, W
            class_ = class_.repeat(A, 1, 1).reshape(-1) # A * H * W
            thresh_idx = conf_score > thresh
            conf_score = conf_score[thresh_idx]
            proposal = proposal[thresh_idx]
            class_ = class_[thresh_idx]
            keep = torchvision.ops.nms(proposal, conf_score, nms_thresh)
            conf_score = conf_score[keep]
            proposal = proposal[keep]
            class_ = class_[keep]
            final_proposals.append(proposal)
            final_conf_scores.append(conf_score.reshape(-1, 1))
            final_class.append(class_.reshape(-1, 1))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def nms(boxes, scores, iou_threshold=0.5, topk=None):
  ...
  suppressed = torch.zeros_like(scores).to(torch.bool)
  keep = []
  for _i in range(len(scores)):
    ...
    if suppressed[i]:
        continue
    keep.append(i)
    i_area = (boxes[i, 2] - boxes[i, 0]) * (boxes[i, 3] - boxes[i, 1])
    if _i < len(scores) - 1:
        j = order[_i+1:]
        jj = ~ suppressed[j]
        j = j[jj]
        j_area = (boxes[j, 2] - boxes[j, 0]) * (boxes[j, 3] - boxes[j, 1])
        dx = (torch.min(boxes[i:i+1, 2], boxes[j, 2]) 
            - torch.max(boxes[i:i+1, 0], boxes[j, 0]))
        dx = torch.clamp(dx, 0)
        dy = (torch.min(boxes[i:i+1, 3], boxes[j, 3]) 
            - torch.max(boxes[i:i+1, 1], boxes[j, 1]))
        dy = torch.clamp(dy, 0)
        inter = dx * dy
        over = inter / (i_area + j_area - inter)
        suppressed[j[over > iou_threshold]] = True
  keep = torch.Tensor(keep).to(torch.long)
  if topk is not None:
    keep = keep[:topk]
